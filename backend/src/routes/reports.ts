...
router.get('/analytics', authenticate, async (req: Request, res: Response): Promise<any> => {
  ...
  try {
    ...
    const revenueProfitDataMap: Record<string, { month: string; revenue: number; profit: number }> = {};
    for (let i = 5; i >= 0; i--) {
    ...
    allExpenses.forEach(e => {
      ...
      if (revenueProfitDataMap[mStr]) revenueProfitDataMap[mStr].profit -= e.amount;
    });
    allPurchases.forEach(p => {
      const mStr = p.date.toLocaleString('default', { month: 'short' });
      if (revenueProfitDataMap[mStr]) revenueProfitDataMap[mStr].profit -= p.total;
    });
