...
router.get('/', authenticate, async (req, res) => {
    ...
    try {
        ...
        const lastMonthOrders = await prisma.order.findMany({
            where: { restaurantId, createdAt: { gte: startOfLastMonth, lt: startOfThisMonth } }
        });
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const dOrders = await prisma.order.findMany({ where: { restaurantId, createdAt: { gte: startOfDay, lte: endOfDay } } });
